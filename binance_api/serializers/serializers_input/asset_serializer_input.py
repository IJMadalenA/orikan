...
class AssetSerializerInput(BaseBinanceSerializerInput):
    ...
    def to_internal_value(self, data):
        data['min_withdraw_amount'] = data.pop('minWithdrawAmount', None)
        data['deposit_status'] = data.pop('depositStatus', None)
        data['withdraw_fee'] = data.pop('withdrawFee', None)
        data['withdraw_status'] = data.pop('withdrawStatus', None)
        data['deposit_tip'] = data.pop('depositTip', None)
        data['deposit_all_enable'] = data.pop('depositAllEnable', False)
        data['withdraw_all_enable'] = data.pop('withdrawAllEnable', False)
        data['is_legal_money'] = data.pop('isLegalMoney', False)
