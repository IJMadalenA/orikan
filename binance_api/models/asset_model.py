...
class Asset(BaseBinanceModel):
    ...
    def get_or_create_asset(cls, acronym):
        ...
        """
        try:
            if Asset.objects.filter(acronym=acronym).exists():
                return Asset.objects.get(acronym=acronym)
            else:
                # Normalize the acronym and delete the spaces.
                acronym = acronym.upper()
                acronym = acronym.replace(" ", "")

                asset = cls.__api__().get_asset_details(asset=acronym)

                asset = asset[acronym]
                asset["acronym"] = acronym

                asset = AssetSerializerInput(data=asset)
                if asset.is_valid(raise_exception=True):
                    asset.save()
                    return asset.instance
                else:
                    logging.error(f"Asset with acronym {acronym} could not be created.")
        except Asset.DoesNotExist:
            logging.error(
                "Asset with acronym {} does not exist.".format(acronym)
            )
