...
class AssetAdmin(ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        # Anotar el último registro de actualización de cada activo.
        latest_updates = Asset.objects.values("acronym").annotate(
            latest_update=Max("updated_at")
        )

        # Obtener los acrónimos de los activos actualizados.
        latest_asset_ids = [asset["acronym"] for asset in latest_updates]

        # Filtrar los activos por los acrónimos obtenidos.
        queryset = queryset.filter(acronym__in=latest_asset_ids)
